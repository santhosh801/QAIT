...
window.updateStatus = window.updateStatus || function(id, status){
  console.warn("updateStatus called before definition", {id, status});
  (window._queuedUpdateStatusCalls = window._queuedUpdateStatusCalls || []).push({id, status});
};
...
window.initTopScrollbars = window.initTopScrollbars || function(){
  if (typeof window._real_initTopScrollbars === 'function') {
    try { window._real_initTopScrollbars(); } catch(e){ console.warn('initTopScrollbars stub forward failed', e); }
  }
};

window.updateStatus = window.updateStatus || function(id, status){
  console.warn('updateStatus called before definition', {id:id, status:status});
  (window._queuedUpdateStatusCalls = window._queuedUpdateStatusCalls || []).push({id:id, status:status});
};
...
function toast(msg, opts = {}) {
  ...
  if (!cont) {
    ...
    cont.id = containerId;
    cont.style = 'position:fixed;left:50%;transform:translateX(-50%);bottom:28px;z-index:99999;';
    document.body.appendChild(cont);
  }
  const el = document.createElement('div');
  el.className = 'qait-toast';
  el.style = 'min-width:220px;margin:6px;padding:10px 14px;border-radius:8px;box-shadow:0 6px 18px rgba(0,0,0,0.12);font-weight:600;opacity:0;transition:opacity .35s,transform .35s;';
  // default colors, adapt if opts.type given
  if (opts.type === 'error') el.style.background = '#ffe6e6', el.style.color = '#611';
  else if (opts.type === 'success') el.style.background = '#e8f7e9', el.style.color = '#223';
...
try {
  ...
  if (typeof updateStatus === 'function') {
    ...
    if (window._queuedUpdateStatusCalls) {
      window._queuedUpdateStatusCalls.forEach(c => {
        try { window.updateStatus(c.id, c.status); } 
        catch(e){ console.warn("queued updateStatus failed", e); }
      });
  ...
  if (typeof requestResubmission === 'function') window.requestResubmission = requestResubmission;
...
try {
  ...
  if (typeof initTopScrollbars === 'function') window.initTopScrollbars = initTopScrollbars;
  if (typeof updateStatus === 'function') {
    window.updateStatus = updateStatus;
    if (window._queuedUpdateStatusCalls) {
      window._queuedUpdateStatusCalls.forEach(c => { try { window.updateStatus(c.id, c.status); } catch(e) { console.warn(e); } });
      window._queuedUpdateStatusCalls = [];
    }
  }
  if (typeof acceptRow === 'function') window.acceptRow = acceptRow;
