...
$labels = [
  ...
  'permanent_address_proof_file' => 'addressproofdoc',
  'nseit_cert_file' => 'nseitcertdoc',
  'self_declaration_file' => 'selfdecdoc',
  'non_disclosure_file' => 'ndadoc',
  'police_verification_file' => 'policeverdoc',
  'parent_aadhar_file' => 'parentaadhardoc',
...
<body>
  <div class="container">
    ...
    <form id="msform" method="post" action="operator_view.php" enctype="multipart/form-data">
      <!-- Personal Info -->
      <fieldset>
        <input type="text" name="operator_full_name" placeholder="Full Name as per in Aadhaar ID" required>
      ...
      $.ajax({
        ...
        if (!resp || !resp.success) {
          ...
          if (confirm('Failed to create resubmission request. Send mail based on rejection_summary anyway?')) {
            $.post('send_rejection_mail.php', {
              id: opId
            }, function(r) {
              if (r && r.success) alert('Mail sent');
        ...
        alert('Failed to create resubmission request');
      });
    }
